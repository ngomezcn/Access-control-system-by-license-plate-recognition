...
function deletePost(id) {
  fetch("http://localhost:8080/api/v1/authcars?id=" + id, {
    method: "DELETE",
  })
    .then(async (response) => {
...
class PlatesTable extends React.Component {
  ...
  componentDidMount() {
    fetch("http://localhost:8080/api/v1/authcars")
      .then((res) => res.json())
      .then((json) => {
  ...
  activateRelay(id) {
    if (id == 0) {
      this.setState({ relay0: "Activando..." });
      setTimeout(
        function () {
          this.setState({ relay0: "Abrir entrada" });
        }.bind(this),
        2050
      );
    }
    if (id == 1) {
      this.setState({ relay1: "Activando..." });
      setTimeout(
        function () {
          this.setState({ relay1: "Abrir salida" });
        }.bind(this),
        2050
      );
    }

    fetch("http://localhost:8080/api/v1/activateRelay?id=" + id)
      .then((res) => res.json())
      .then((json) => {
        this.setState({});
      });
  ...
  render() {
    ...
    const { relay0 } = this.state;
    const { relay1 } = this.state;
  ...
  editar = (dato) => {
    ...
    fetch("http://localhost:8080/api/v1/authcars", requestOptions)
